...
void lcd_cmd(unsigned char a)  //Function to send COMMAND into the LCD
{
    IO0PIN&=0x00;                  
    IO0PIN|=(a<<16);
    IO0CLR|=bit(25);           //To send the COMMANDS into the LCD, Register Select has to be 0 ; rs=0
    IO0CLR|=bit(26);           //For writing into the LCD rw=0
    IO0SET|=bit(27);          //To latch the command/data sent into the LCD a high to low pulse is given; en=1
    delay();                  //Call a delay
    IO0CLR|=bit(27);          //en=0
}
...
void lcd_data(unsigned char b)  //Function to send DATA into the LCD
{
    IO0PIN&=0x00;
    IO0PIN|=(b<<16);
    IO0SET|=bit(25);           //To send the DATA into the LCD, Register Select has to be 1 ; rs=1
    IO0CLR|=bit(26);           //For writing into the LCD rw=0
    IO0SET|=bit(27);          //To latch the command/data sent into the LCD a high to low pulse is given; en=1
    delay();                  //Call delay
    IO0CLR|=bit(27);          //en=0
